...
def send_tcp_target(robot, pose, prev_pose=None, dt=None,
    ...
    SendCartesianMotionForce accepts a 6-DOF velocity plus per-axis
    max_linear_vel/max_angular_vel caps. Passing the desired velocity derived
    from (pose - prev_pose)/dt makes the arm move at the commanded speed
    instead of relying on its internal (slower) trajectory interpolation,
    which was the cause of the sluggish speed following.
    """
    if prev_pose is not None and dt is not None and dt > 1e-6:
        vel = np.zeros(6)
        vel[:3] = (pose[:3] - prev_pose[:3]) / dt
        # angular velocity from quaternion difference (small-angle approx)
        Rc = quat_to_mat(prev_pose[3:7])
        Rt = quat_to_mat(pose[3:7])
        rv = matrix_to_rotvec(Rt @ Rc.T)
        vel[3:] = rv / dt
        # clamp requested velocity to safe caps
...
def main():
    ...
    vr_ref = None
    tcp_ref = None
    prev_motion = None        # previous commanded target (for feed-forward vel)
            ...
            if not args.dry_run and motion_target is not None:
                try:
                    if not robot.operational() or not robot.estop_released():
                        ...
                        loop_now = time.time()
                        dt = (loop_now - prev_loop_t) if prev_loop_t is not None else 0.0
