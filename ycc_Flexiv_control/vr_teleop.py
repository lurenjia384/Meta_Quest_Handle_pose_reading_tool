...
FPS = 100.0   # high-frequency streaming setpoint rate (doc recommends 100Hz)
...
class ReferenceGovernor:
    ...
    def __init__(self, tau=0.10,
        ...
        # FIXED governor control period (100Hz). The governor ALWAYS
        # integrates at this rate regardless of how often update() is called.
        # External calls at 60Hz or 1000Hz both produce identical dynamics:
        # we accumulate real time and execute an integer number of 10ms steps.
        self.dt = dt
        self._acc = 0.0
        self.p_cmd = None
    ...
    def _step(self, dt):
        # translation
        error = self.target_pose[:3] - self.p_cmd
        ...
        # rotation
        q_t = self.target_pose[3:7] / np.linalg.norm(self.target_pose[3:7])
    ...
    def update(self, target_pose, dt=None):
        """Advance the governor by real elapsed time dt.

        Accumulates dt and executes whole 10ms control steps, so dynamics are
        identical whether update() is called at 60Hz or 1000Hz.
        """
        ...
        if dt is None:
            dt = 1.0 / 100.0
        dt = max(0.0, min(dt, 0.05))  # clamp to avoid huge jumps on hiccups
        self._acc += dt
        while self._acc >= self.dt - 1e-9:
            self._step(self.dt)
            self._acc -= self.dt
...
def send_tcp_target(robot, pose, vel=None, max_lin_vel=0.15, max_ang_vel=5.0):
    """Send a Cartesian target with an explicit feed-forward velocity.

    vel is the governor's commanded velocity (6-DOF: 3 linear + 3 angular),
    which is already velocity/acceleration-limited and dt-independent. Using
    it (instead of (pose-prev)/dt) avoids the velocity explosion at high
    call rates and makes the arm move at the commanded speed.
    """
    if vel is not None:
...
def main():
    ...
    vr_ref = None
    tcp_ref = None
    governor = ReferenceGovernor()   # velocity/acceleration-continuous limiter
    prev_loop_t = None               # last governor update timestamp
    prev_b = False
    prev_trigger = False
    ...
    try:
        while not stop_event.is_set():
            ...
            if b and not prev_b:
                ...
                governor.reset(cur_pose)        # start governor from real TCP
                b_was_active = True
            ...
            # --- high-frequency streaming setpoint ---------------------------
            # The servo loop runs at FPS (100Hz) and ALWAYS streams a target,
            # independent of VR data freshness. When B is held and VR data is
            # fresh, the absolute mapping + governor produce a velocity-limited
            # streamed target. When VR data is stale while B is held, the
            # governor target is held constant (velocity decays to 0 smoothly),
            # but we keep streaming so the arm never sees a command gap.
            loop_now = time.time()
            dt_gov = (loop_now - prev_loop_t) if prev_loop_t is not None else 1.0 / FPS
            prev_loop_t = loop_now

            motion_target = None
            cmd_vel = None
            if b and rp is not None and vr_ref is not None and tracking_ok:
                if data_fresh:
                    # absolute mapping -> raw target
                    tcp_target = np.empty(7)
                    tcp_target[:3] = (tcp_ref[:3]
                                      + axis_matrix @ (rp[:3, 3] - vr_ref[:3, 3]) * pos_scale)
                    R_vr_ref = vr_ref[:3, :3]
                    R_rp = rp[:3, :3]
                    R_vr_delta = R_rp @ R_vr_ref.T          # handle relative rot
                    ...
                    # VR data stale but B held: keep the last raw target fixed,
                    # let the governor's internal velocity decay to zero so the
                    # arm decelerates smoothly instead of jumping or continuing.
            ...
            # always stream: send every loop tick, even in hold
                ...
                try:
                    if not robot.operational() or not robot.estop_released():
                        ...
                        if cmd_vel is not None:
                            send_tcp_target(robot, motion_target, cmd_vel)
                        else:
                            send_tcp_target(robot, motion_target)
                        if args.verbose_motion and b and data_fresh:
